...
class Posts{
	...
	function searchPostsTag($tema){
			//echo "muchos";
			$sql="select id_post, nom_tema from post_temas, temas where post_temas.id_tema = temas.id_tema and nom_tema ='$tema'";
			$querypost1 = mysqli_query($this->conexion, $sql);
			while($post1=mysqli_fetch_assoc($querypost1)){
				$this->idpost[] 			=  $post1["id_post"];
				$this->tema[] 		=  $post1["nom_tema"];
				$sql = "select id_post, titulo, `like`, alias_usuario, (time_to_sec(datediff(now(),fecha))) as fecha from crear_post, usuarios where crear_post.id_user=usuarios.id_user and id_post=".$post1['id_post']."";
				$querypost = mysqli_query($this->conexion, $sql);
...
/*select id_post, nom_tema from post_temas, temas where post_temas.id_tema = temas.id_tema and nom_tema =nom_tema*/
